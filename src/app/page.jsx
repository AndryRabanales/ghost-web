// src/app/page.jsx (Versión Final con Fondo de Imagen Local y Corrección)
"use client";
...
export default function Home() {
  ...
  const colors = useMemo(() => ({
    primaryRed: '#FE3C72', secondaryPink: '#FF655B', darkBg: '#121212',
    cardBg: '#1A1A1A', textPrimary: '#FFFFFF', textSecondary: 'rgba(235, 235, 245, 0.6)',
    inputBg: '#2C2C2E', inputBorder: '#48484A',
  }), []);

  const dynamicStyles = `
    @keyframes fadeInUp { from { opacity: 0; transform: translateY(30px); } to { opacity: 1; transform: translateY(0); } }
    @keyframes pulse-glow { 0% { box-shadow: 0 0 25px ${colors.primaryRed}33; } 50% { box-shadow: 0 0 40px ${colors.primaryRed}88; } 100% { box-shadow: 0 0 25px ${colors.primaryRed}33; } }
  ...
  const pageStyle = {
    ...
    backgroundImage: 'url(/background-home.png)', // Carga la imagen desde la carpeta /public
    backgroundSize: '120% 120%', // Zoom para que el paneo se vea bien
    backgroundPosition: 'center center',
    animation: 'background-pan 40s linear infinite alternate', // Animación de movimiento lento
  };

  const cardStyle = {
    position: 'relative', zIndex: 2, width: '100%', maxWidth: '450px',
    padding: '45px 40px', background: `radial-gradient(circle at 50% 0%, rgba(254, 60, 114, 0.1), transparent 70%), ${colors.cardBg}e6`,
  ...
  // --- LÓGICA DE ESTILO DEL BOTÓN CORREGIDA ---
  const getButtonStyle = () => {
    let style = {
      width: '100%', padding: '20px', marginTop: '20px', background: `linear-gradient(90deg, ${colors.secondaryPink}, ${colors.primaryRed})`,
      ...
      display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px', boxShadow: `0 10px 30px ${colors.primaryRed}55`,
    ...
    <>
      ...
      <div style={pageStyle}>
        <main style={cardStyle}>
            <h1 style={{...{ color: colors.textPrimary, fontSize: '44px', fontWeight: '800', letterSpacing: '-2.5px', background: `linear-gradient(90deg, ${colors.secondaryPink}, ${colors.primaryRed})`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', margin: '0 0 15px' }, ...(isMounted ? {animation: 'fadeInUp 0.8s cubic-bezier(0.2, 0.8, 0.2, 1) forwards', animationDelay: '0.3s'} : {opacity: 0})}}>
              ...
              <div style={isMounted ? {animationName: 'fadeInUp', animationDelay: '0.7s', opacity: 0} : {opacity:0}} className="staggered-fade-in-up">
                <input type="text" placeholder="Escribe tu nombre (opcional)" value={name} onChange={(e) => setName(e.target.value)} onFocus={() => setIsFocused(true)} onBlur={() => setIsFocused(false)}
                  style={{
                    ...
                    border: `2px solid ${isFocused ? colors.primaryRed : 'transparent'}`,
                    borderRadius: '16px', color: colors.textPrimary, fontSize: '18px',
                    textAlign: 'center', outline: 'none', boxSizing: 'border-box',
                    transition: 'all 0.3s ease', boxShadow: isFocused ? `0 0 30px ${colors.primaryRed}66` : '0 4px 15px rgba(0,0,0,0.4)',
                  }}
                />
              </div>
            ...
            {error && <p style={{ color: colors.secondaryPink, marginTop: '20px' }}>{error}</p>}
